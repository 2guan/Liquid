...
export default function RankBadge({ compact = false }: { compact?: boolean }) {
  ...
  return (
    <div className="flex items-center gap-3">
      {/* avatar medallion */}
      <div className="relative grid h-11 w-11 shrink-0 place-items-center rounded-full border border-gold/50 bg-gradient-to-b from-wood-light to-wood-dark shadow-amber-soft">
      ...
      {!compact && (
        <div className="min-w-[150px]" suppressHydrationWarning>
          <div className="flex items-baseline justify-between gap-2">
            <span className="font-cn text-sm text-paper">{mounted ? meta.name : " "}</span>
            <span className="font-ui text-[11px] text-gold/80">{mounted ? `Lv.${level}` : " "}</span>
          </div>
          <p className="font-title text-[9px] uppercase tracking-title text-gold/55">{mounted ? meta.nameEn : " "}</p>
          {/* xp bar */}
          <div className="mt-1.5 h-1.5 w-full overflow-hidden rounded-full bg-ink-soft/80">
            <div
          ...
          <p className="mt-0.5 text-right font-ui text-[9px] text-paper/40">

...
export interface LayerBand {
  ...
  body: string;
  shadow: string;
  blendUp: string; // colour at the band's top edge (toward the layer above)
  blendDown: string; // colour at the band's bottom edge (toward the layer below)
}
 ...
 * hence no antialiased seam lines (the Android "two lines" bug). Adjacent bands
 * meet at a shared blended colour, so the gradient stays seamless.
 */

 ...
 *
 * The default export is an offline `MockCocktailAI` that composes structured
...
export class RemoteCocktailAI implements CocktailAI {
  ...
  private async post<T>(path: string, body: unknown): Promise<T> {
    ...
    if (!res.ok) throw new Error(`AI endpoint ${path} failed: ${res.status}`);
...
/**
 * HybridCocktailAI — what the app actually uses. It calls the real LLM via the
 * /api routes and transparently falls back to the offline poet on any failure
 * (network down, route missing in a static export, malformed reply), so the
 * experience never breaks.
 */
class HybridCocktailAI implements CocktailAI {
  private remote = new RemoteCocktailAI(
    (process.env.NEXT_PUBLIC_AI_ENDPOINT ?? "/api").replace(/\/$/, ""),
  );
  ...
  analyzeFlavorMix(picks: FlavorPick[]) {
    // Garnish-only / zero-proof creations still go to the LLM — the server prompt
    // (dsZenMix) instructs it to write a witty card that's honest about being
    // non-alcoholic / not-a-drink. The offline poet stays as the fallback and
    // produces the same honest framing if the LLM is unavailable.
...
/** The app imports this. Real DeepSeek LLM via /api, offline poet as fallback. */
export const cocktailAI: CocktailAI = new HybridCocktailAI();

/** Server-side offline fallback used by the /api route handlers. */

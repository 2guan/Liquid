...
function drawGlass(ctx: any, result: CocktailResult, hi: string, body: string, shadow: string, topY: number, targetH: number, fizzy: boolean, garnishes: GarnishSpec[]): void {
  ...
  if (ice !== "none") {
    ...
    drawIce(ctx, ice as any, 100, iceY, iceR, liquidTop, body, liquidTop, geom.cup.bottom, interiorHW, result.iceSeed);
    ctx.restore();
  }
...
const SHARDS: [string, number][] = [
  ["M-30,12 Q-20,-4 0,6 Q-8,22 -16,24 Z", 0.5], ["M-6,-8 Q10,-22 26,-4 Q17,7 8,8 Z", 0.62],
  ["M2,2 Q20,-8 30,10 Q19,20 12,18 Z", 0.46], ["M-18,18 Q0,8 12,24 Q2,34 -6,32 Z", 0.55],
  ["M-34,22 Q-18,14 -10,30 Q-22,38 -26,36 Z", 0.42], ["M14,12 Q30,6 34,24 Q24,30 18,28 Z", 0.5],
  ["M-12,28 Q6,20 16,36 Q4,44 -2,42 Z", 0.4], ["M-2,-16 Q12,-26 22,-12 Q13,-2 8,-4 Z", 0.7],
  ["M20,20 Q34,16 32,34 Q22,36 18,34 Z", 0.36], ["M-26,4 Q-14,-4 -6,10 Q-16,18 -20,16 Z", 0.58],
];

function drawIce(ctx: any, type: string, cx: number, cy: number, r: number, waterY: number, liquidColor: string, fillTop?: number, fillBottom?: number, fillHW?: number, iceSeed = 0): void {
  const tint = "#e3edf2";
  if (type === "cubes" || type === "bullets") {
    drawIceFill(ctx, type, cx, waterY, liquidColor, fillTop != null ? fillTop : cy - r, fillBottom != null ? fillBottom : cy + r, fillHW != null ? fillHW : r, iceSeed);
    return;
  }
  const wl = waterY - cy;
  ctx.save();
  ...
  if (type === "sphere") {
    ...
    ctx.save();
    if (ctx.filter !== undefined) ctx.filter = "blur(1.8px)";
    ctx.fillStyle = withAlpha(tint, 0.07); ellipse(ctx, 0, 14, 36, 26); ctx.fill();
    ctx.restore();
    ...
    for (const [d, op] of SHARDS) {
      ctx.save();
      if (ctx.filter !== undefined) ctx.filter = "blur(0.75px)";
      ctx.globalAlpha = 0.52 + op * 0.1;
      trace(ctx, d); ctx.fillStyle = grad; ctx.fill();
      ctx.restore();
      ctx.save();
      if (ctx.filter !== undefined) ctx.filter = "blur(0.8px)";
      trace(ctx, d); ctx.strokeStyle = "rgba(255,255,255,0.1)"; ctx.lineWidth = 0.55; ctx.stroke();
      ctx.restore();
    }
    ctx.save();
    if (ctx.filter !== undefined) ctx.filter = "blur(1.8px)";
...
function drawIceFill(ctx: any, type: string, cx: number, waterY: number, liquidColor: string, top: number, bot: number, hw: number, iceSeed = 0): void {
  const tint = "#e3edf2";
  ...
  const pCubeRight = "M 0,-1 L 10.9,-7.3 Q 12.5,-8.2 12.5,-6.4 L 12.5,4.4 Q 12.5,6.2 10.9,7.1 L 1.6,12.5 Q 0,13.4 0,11.6 Z";
  const pCubeLeft = "M 0,-1 L -10.9,-7.3 Q -12.5,-8.2 -12.5,-6.4 L -12.5,4.4 Q -12.5,6.2 -10.9,7.1 L -1.6,12.5 Q 0,13.4 0,11.6 Z";
  const pCubeTop = "M -12.5,-8.2 L -1.6,-14.5 Q 0,-15.4 1.6,-14.5 L 12.5,-8.2 L 1.6,-1.9 Q 0,-1 -1.6,-1.9 Z";
  const pCubeCoreTop = "M -4.5,-3.9 L -0.7,-6.1 Q 0,-6.5 0.7,-6.1 L 4.5,-3.9 L 0.7,-1.7 Q 0,-1.3 -0.7,-1.7 Z";
  const pCubeCoreLeft = "M 0,-1.3 L -3.9,-3.6 Q -4.5,-3.9 -4.5,-3.2 L -4.5,1.3 Q -4.5,2 -3.9,2.4 L -0.7,4.2 Q 0,4.6 0,3.9 Z";
  const pCubeCoreRight = "M 0,-1.3 L 3.9,-3.6 Q 4.5,-3.9 4.5,-3.2 L 4.5,1.3 Q 4.5,2 3.9,2.4 L 0.7,4.2 Q 0,4.6 0,3.9 Z";
